...
class Solution(object):
    def __init__(self, s=None):
        if isinstance(s, Graph):
            self.g = s
            self.cost = 0
            self.visited = list(np.zeros((s.N,), dtype=np.int))
            
            self.not_visited = np.arange(1,s.N)
            self.index_next = 0
    ...
    def add_edge(self, v, u):
        ...
        self.visited[self.index_next]= u 
        self.index_next+=1
        if (u!=0):
            self.not_visited = np.delete(self.not_visited,np.where(self.not_visited==u))
