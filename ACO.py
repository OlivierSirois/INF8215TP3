...
class ACO(object):
       ...
       def get_next_city(self, sol):
              ...
              b = self.parameter_beta  # beta - relative importance of pheromone vs distance        
              ...
              if len(sol.not_visited) > 1:
                     ...
                     return np.random.choice(nv, 1, p=prob[nv])       # random next city 
       ...
       def heuristic2opt(self, sol):
              ...
              for i in range(-1, len(sol.visited)-1):
                     for j in range(i+1, len(sol.visited)-1): # non-consecutive city
